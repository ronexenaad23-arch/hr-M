...
if [ -z "$U" ] || [ -z "$T" ]; then
  echo "الاستخدام: bash deploy.sh USERNAME TOKEN"
  echo "TOKEN من: github.com/settings/tokens/new → repo"
  exit 1
fi
echo "🏥 نشر نظام الموارد الطبية..."
...
git add -A && git commit -q -m "HR System v5 $(date '+%Y-%m-%d')" 2>/dev/null || true
...
echo "✅ تم: https://github.com/$U/$R"
echo "📌 Netlify: app.netlify.com → New site → Import from GitHub → $R → Publish: public → Deploy"
